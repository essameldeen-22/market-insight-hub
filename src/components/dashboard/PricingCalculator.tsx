...
export function PricingCalculator({ currency }: { currency: Currency }) {
  ...
  const optimal = state.customers === 0 ? 0 : Math.round(((state.cost / state.customers) * (1 + state.margin / 100)) / 5) * 5;
  ...
  const doExportCsv = () => {
    const rows: (string | number)[][] = [
      ["Metric", "Value"],
      ["Monthly cost", state.cost],
      ["Expected customers", state.customers],
      ["Competitor price", state.competitor],
      ["Target margin %", state.margin],
      ["Model", state.model],
      ["Optimal price", optimal],
      ["Monthly revenue", revenue],
      ["Monthly profit", profit],
      ["Annual revenue", annual],
      ["Break-even customers", breakEven],
    ];
    exportToCsv("pricing-calculator.csv", rows);
  };
